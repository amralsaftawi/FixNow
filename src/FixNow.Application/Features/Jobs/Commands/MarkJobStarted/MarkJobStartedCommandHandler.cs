...
public sealed class MarkJobStartedCommandHandler(
    ITechnicianProfileRepository technicianProfileRepository,
    IJobRepository jobRepository,
    IAssignmentRepository assignmentRepository,
    ICurrentUser currentUser)
    : ICommandHandler<MarkJobStartedCommand, Result<Success>>
{
    public async Task<Result<Success>> Handle(
        MarkJobStartedCommand command,
        CancellationToken cancellationToken)
    {
        ...
        //    technician profile cannot start a job.
        ...
        // 3. Only the technician assigned to the job may start it. An
        ...
        //    owns the valid-transition rules (Arrived -> InProgress is the
        //    start transition); the client can never force an arbitrary
        //    status change.
        var transitionResult = job.ChangeStatus(JobStatus.InProgress);
