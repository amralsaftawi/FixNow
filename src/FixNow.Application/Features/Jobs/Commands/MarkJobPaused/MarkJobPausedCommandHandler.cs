...
public sealed class MarkJobPausedCommandHandler(
    ITechnicianProfileRepository technicianProfileRepository,
    IJobRepository jobRepository,
    IAssignmentRepository assignmentRepository,
    ICurrentUser currentUser)
    : ICommandHandler<MarkJobPausedCommand, Result<Success>>
{
    public async Task<Result<Success>> Handle(
        MarkJobPausedCommand command,
        CancellationToken cancellationToken)
    {
        ...
        //    technician profile cannot pause a job.
        ...
        // 3. Only the technician assigned to the job may pause it. An
        ...
        //    owns the valid-transition rules (InProgress -> Paused is the
        //    pause transition); the client can never force an arbitrary
        //    status change.
        var transitionResult = job.ChangeStatus(JobStatus.Paused);
