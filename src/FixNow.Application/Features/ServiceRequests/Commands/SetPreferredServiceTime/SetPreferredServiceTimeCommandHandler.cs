...
public sealed class SetPreferredServiceTimeCommandHandler(
    ICustomerRepository customerRepository,
    IServiceRequestRepository serviceRequestRepository,
    ICurrentUser currentUser)
    : ICommandHandler<SetPreferredServiceTimeCommand, Result<Success>>
{
    public async Task<Result<Success>> Handle(
        SetPreferredServiceTimeCommand command,
        CancellationToken cancellationToken)
    {
        ...
        // 4. Apply the preferred service time through the domain model.
        var scheduleResult = serviceRequest.Schedule(
            command.PreferredServiceTime);
